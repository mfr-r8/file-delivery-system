...
def load_all_students():
    ...
    # عرض معلومات تشخيصية
    if not files:
        st.warning("⚠️ لم يتم العثور على أي ملفات Google Sheets في المجلد")
        return pd.DataFrame()
    ...
    for f in files:
        ...
        if not raw_values:
            files_summary.append(f"❌ {f['name']}: فارغ أو غير قابل للقراءة")
            continue
        ...
        files_summary.append(f"✅ {f['name']}: {len(students)} طالب ({len(raw_values)} صف)")
    
    # عرض الملخص للمدير والمشرف فقط
    user = st.session_state.get("user")
    if user and user.get("role") in ["مدير", "مشرف"]:
        with st.expander("🔍 تفاصيل تحميل الملفات (للتشخيص)"):
            for line in files_summary:
                st.write(line)
            st.write(f"**المجموع الكلي: {len(all_students)} طالب**")
